...
도로변, 공원, 지하주차장에 방치된 자전거는 도시 미관과 안전을 해치고,
자원으로도 버려지고 있습니다.

**2WheelAgain**은 방치자전거의 전 과정을 플랫폼으로 연결합니다.

> **1단계 (MVP)** — 시민 제보 → 지자체 법적 절차 → BikeRun 수거 · 수리 · 게시 · 매칭 (연락 / 오프라인 거래 중심)
> **2단계** — 온라인 결제, AI 추천 / 매칭 서비스, 모바일 앱 (안드로이드 · iOS) 확장

#### 1.2.1 1단계 핵심 흐름 (MVP)

```
[시민] — 방치자전거 사진·위치 제보(신고) → 플랫폼
    ↓
[지방자치단체 (시·군·구)] — 방치자전거 처리 법적절차 확인 → 공고 → 수거
    ↓
[BikeRun (사회적기업)] — 수거된 자전거 전달 → 상태 점검 · 수리 · 플랫폼 게시
    ↓
[시민] — 자전거 목록 확인 → 문의/나눔 신청 → 연락처 교환 → 직접 전달 (오프라인)
```

**핵심 원칙**:
- 1단계에서는 **온라인 결제·결제 기능을 포함하지 않음**. 거래는 시민 간 직접 연락 및 오프라인 체결.
- 방치자전거 제보는 누구나 가능하지만, **수리·등록·게시·매칭은 오직 BikeRun 관리자만** 수행.
- 지자체와의 연동은 **법적 처리 절차**에 초점 — 수거 일정·처리 결과 통보.

#### 1.2.2 2단계 계획 (V2+)

| 항목 | 내용 |
|------|------|
| **온라인 결제** | 자전거 구매 시 카드·계좌이체·가상화폐 결제 지원 |
| **AI 추천·매칭** | 사용자 행동 데이터 기반 맞춤 자전거 추천, 구매자-판매자 자동 매칭 |
| **모바일 앱** | 안드로이드 (Google Play), iOS (App Store) —-push 알림, GPS 위치기반 제보, 카메라 기반 사진 촬영 |
| **카카오톡 알림** | 예약 상태 변경, 매칭 성공 시 카카오톡 알림톡 발송 |
| **고도화** | 후기_rating 시스템 고도화, 지역별 맞춤 필터, 구독 알림 |

### 1.3 목표 KPI (1차 출시 기준 — V1 MVP)
...
| 분반모율 | 5% 미만 | 미매칭 |
| 방치자전거 제보 수 | 200건 / 월 | 시민 제보 활성도 |
| 제보 → 수거 전환율 | 70% 이상 | 지자체·BikeRun 협력도 |
...
| **C (일반 시민 — 구매/나눔)** | 누구나 | 자전거 구매 또는 나눔 신청 · 예약 · 체결 |
| **R (제보자)** | 로그인한 모든 사용자 | 방치자전거 제보(신고), 제보 상태 추적 |

> **핵심 원칙**: 자전거 등록 · 수리 · 판매/나눔 게시는 오직 **BikeRun 관리자**만 수행합니다.
> 일반 시민은 등록이 불가능하며, 목록 확인 · 문의 · 예약 · 체결 · 제보만 가능합니다.
...
### F8. 방치자전거 제보 / 신고 (Citizen Report)

**목적**: 시민이 도로변, 공원 등에 방치된 자전거를 플랫폼을 통해 신고하면, 지자체 법적 절차 거쳐 BikeRun 수거·처리 연동

> **접근 제한: 로그인한 모든 사용자 접근 가능 (비로그인은 제보 작성 불가)**

#### 4.8.1 제보 쓰기

| 필드 | 타입 | 필수 | 비고 |
|------|------|------|------|
| 자전거 유형 | radio | 예 | 방치·버려짐, 폐기예정,捐赠가능, 기타 |
| 사진 | upload | 예 | 최소 1장, 최대 3장 (JPG/PNG, 1장당 5MB) |
| 위치 (주소) | text + map | 예 | 주소 검색 또는 지도 핀 클릭 |
| GPS 위치 | auto-detect | 아니요 | 모바일 앱에서는 자동 수집 |
| 상세 설명 | textarea | 아니요 | 최대 1,000자 (현황, 주변 참고사항) |
| 제보일시 | auto | 예 | 제출 시점 자동 기록 |

#### 4.8.2 제보 처리 흐름

```
[시민] -> [GET /reports/new] -> [ReportForm (Client)]
    -> [POST /api/reports] -> [대기 중]
    ↓
[BikeRun 관리자] -> [GET /admin/reports] -> [제보 목록 확인]
    -> [수거 요청 발송 -> 지자체 통보]
    ↓
[지자체 (시·군·구)] -> [법적 처리 절차 진행 (공고, 이의제기 기간)]
    -> [수거 완료 -> BikeRun 통보]
    ↓
[BikeRun] -> [자전거 전달 받음 -> 상태 점검 -> 수리 -> F2 등록]
```

#### 4.8.3 제보 관리 대시보드 (/admin/reports)

| 기능 | 설명 |
|------|------|
| 제보 목록 | 상태(대기/처리중/수거완료), 등록일, 지역별 필터 |
| 상태 전이 | 대기 → 처리중 → 수거완료 → 자전거등록 |
| 수거 배정 | BikeRun 담당자 배정, 수거 예정일 설정 |
| 수거 결과 | 수리 전 사진, 상태 메모 입력 |

#### 4.8.4 제보 상태 전이

| 상태 | 설명 |
|------|------|
| `pending` | 제보 접수 완료, BikeRun 확인 대기 |
| `reviewing` | BikeRun 검토 중 |
| `sent_to_municipality` | 지자체 법적 처리 요청 발송 |
| `collecting` | 지자체 수거 진행 중 |
| `collected` | BikeRun 수거 완료 |
| `registered_as_bike` | 자전거 등록 완료 (F2 연동) |
| `closed` | 처리 완료 (비활성화) |
...
BikeReport (방치자전거 제보)
├── id (UUID)
├── reporter_id (UUID → User) — 제보한 시민
├── status (enum: pending, reviewing, sent_to_municipality, collecting, collected, registered_as_bike, closed)
├── bike_type (enum: abandoned, discarded, donation_possible, other)
├── location_address (string) — 주소
├── location_latitude (float, nullable) — GPS 위도
├── location_longitude (float, nullable) — GPS 경도
├── description (string, nullable) — 상세 설명
├── photos (JSON) — [{ url, order }]
├── municipality (string, nullable) — 처리 담당 지자체 (시·군·구)
├── collected_at (datetime, nullable) — 수거일
├── assigned_bike_id (UUID → Bike, nullable) — 매핑된 자전거 (수거 후 등록 시)
├── internal_note (string, nullable) — BikeRun 내부 메모
├── created_at (datetime)
└── updated_at (datetime)
```
...
## 8. 향후 확장 로드맵 (Roadmap — V1 / V2)

### 8.1 V1 (1단계 · MVP) — M1–M3

> **핵심 목표**: 시민 제보 → 지자체 처리 → BikeRun 수리·매칭의 전 과정을 플랫폼으로 연동
> **거래 방식**: **연락 / 오프라인 거래 중심** (온라인 결제 X)

| Phase | 예상 시기 | 주요 기능 |
|-------|-----------|-----------|
| **V1-M1** | M1–M2 | · SNS OAuth 로그인 (네이버·카카오톡) <br> · 플랫폼 소개 (Landing) 페이지 <br> · 자전거 목록·상세 (F2~F4) <br> · 판매 문의 / 나눔 신청 (F5) <br> · 예약·매칭 관리 (F6) <br> · BikeRun 관리자 대시보드 (등록·수리·매칭) |
| **V1-M2** | M2–M3 | · **방치자전거 제보/신고 (F8)** <br> · 제보 처리 대시보드 (대기 → 처리중 → 수거완료) <br> · 지자체 통보 연동 (수거 일정·결과) <br> · 수리 이야기 (Blog) — 수리 스토리 공개 <br> · 완료 페이지·후기 (F7) |
| **V1-M3** | M3 | · 반응형 UI 완성 (Mobile ≥ 375px) <br> · 접근성 WCAG 2.1 AA 준수도 테스트 <br> · 성능 검증 (LCP ≤ 2.5초) <br> · 베타 서비스·피드백 수집 |

### 8.2 V2 (2단계) — M4–M6

> **핵심 목표**: 온라인 결제 · AI 추천 / 매칭 · 모바일 앱 출시로 서비스 고도화

| 항목 | 주요 기능 | 상세 내용 |
|------|-----------|-----------|
| **온라인 결제** | 카드·계좌이체·가상화폐 | · V1 오프라인 거래 + 온라인 결제 병행 <br> · 결제 게이트웨이 연동 (PG사) <br> · 환불·환전 처리 프로세스 |
| **AI 추천·매칭** | 맞춤 추천 알고리즘 | · 사용자 행동 데이터 (조회·문의·예약) 기반 맞춤 자전거 추천 <br> · 위치기반 nearby 자전거 매칭 <br> · 구매 성향 분석 (가격대·유형·지역 선호) |
| **모바일 앱 — 안드로이드** | Google Play 배포 | · React Native / Expo 기반 크로스플랫폼 <br> · 카메라 기반 제보 사진 촬영 <br> · GPS 위치기반 실시간 제보 <br> · 푸시 알림 (예약 상태, 매칭 성공) |
| **모바일 앱 — iOS** | App Store 배포 | · 안드로이드와 동일한 코드베이스 (Expo) <br> · Apple Sign-In 연동 (선택) |
| **카카오톡 알림** | 알림톡 발송 | · 예약 상태 변경 시 자동 알림 <br> · 매칭 성공·일정 변경 알림 <br> · 프로모션·신규 자전거 등록 안내 |
| **고도화** | 후기·평점 시스템 | · 구매 완료 자동 후기 요청 <br> · 자전거 평점 aggregation <br> · 신뢰도 점수 (신고 누적, 후기 수) |

### 8.3 V3 (3단계 · 장기) — M7–M9

| Phase | 예상 시기 | 주요 기능 |
|-------|-----------|-----------|
| **V3-M1** | M7–M8 | · AI 추천 정밀화 (머신러닝 모델 학습) <br> · 지역별 맞춤 커뮤니티 기능 <br> · 자전거 보험·분실 방지 서비스 연동 |
| **V3-M2** | M8–M9 | · 타 지자체·BikeRun 지사 확장 <br> · 데이터 기반 도시 자전거 정책 제안 <br> · ESG· carbon credit 연동 |

---

## 9. 리스크 & 미해결 쟁점 (Risks & Open Issues)
